...
const MenuPideloPe = ({ pages = [], items }) => {
    ...
    // Get only the first 3 featured items
        ...
        <nav className="hidden md:block bg-secondary font-font-secondary font-normal text-sm" ref={menuRef}>
            <div className="px-primary 2xl:px-0 2xl:max-w-7xl mx-auto">
                <ul className="flex items-center gap-6 text-sm">
                    ...
                    {featuredItems.map((page, index) => (
                        <li key={index} className="py-3">
                            <a
                                href={page.path}
                                className="w-full hover:customtext-primary cursor-pointer transition-all duration-300 pr-6 relative before:absolute before:right-0 before:top-1/2 before:-translate-y-1/2 before:h-3 before:w-[1px] before:bg-[#262624]"
                            >
                                {page.name}
                            </a>
                        </li>
