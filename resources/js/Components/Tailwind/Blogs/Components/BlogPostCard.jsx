    ...
    return (
        <article className={`group relative ${featured ? "h-full" : ""}`}>
            <a href="#" className={`block ${flex && 'flex gap-4 '}`} >
                <div className={`relative aspect-video overflow-hidden rounded-lg ${flex && 'aspect-square  w-1/2'} `}>
                    <img
                        src={`/api/posts/media/${post?.image}`}
                        alt={post?.name}
                        fill
                        className={`object-cover transition-transform duration-300 group-hover:scale-105 ${flex && 'h-full w-full'} `}
                    />
                </div>
