  ...
  return (
    <article className={`group relative ${featured ? "h-full" : ""}`}>
      <a href={`/post/${post?.slug}`} className={`block ${flex && "flex gap-4 "}`}>
        <div
          className={`relative aspect-video overflow-hidden rounded-lg ${flex && "aspect-square  w-1/2"
            } `}
        >
          <img
        ...
        <div className={`mt-4 space-y-1 ${flex && "w-1/2 mt-0 gap-2"}`}>
          ...
          <h3 className="text-xl 2xl:text-2xl font-semibold customtext-neutral-dark group-hover:customtext-primary leading-tight">
            {post?.name}
          </h3>
          <p className="line-clamp-3 customtext-neutral-dark opacity-85 text-base 2xl:text-xl leading-tight">
            {post?.summary}
          </p>
